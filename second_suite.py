...
INSTANCES = {
    '3': {
        'cloud_spanner_instance_name': 'pkb-three-node-benchmarks-west2',
    ...
    '6': {
        'cloud_spanner_instance_name': 'pkb-six-node-benchmarks-west2',
    ...
    '9': {
        'cloud_spanner_instance_name': 'pkb-nine-node-benchmarks-west2',
...
def main():
    for test in ['throughput', 'latency']:
        for workload in ['x', 'b', 'a', 'c']:
            parallel_runs = []
            for node_count in INSTANCES:

                tmp_yaml = '/tmp/suite2_{}_{}_{}.yaml'.format(test, workload, node_count)
